...
class Foot4Ever():
    ...
    async def set_admins(self, update, context):
        ...
        """
        if not await self.is_admin(context.bot, update):
            return

        await self.load_users()

        admin_names = ' '.join(context.args)
        self.admins_names = [admin.lower() for admin in admin_names.split(',')]
    ...
    async def load_users(self):
        ...
        self.admins = []
        admins = await self.bot.get_chat_administrators(MAIN_CHAT_ID)
    ...
    async def start(self, update, context):
        ...
        Welcome message
        """
        await context.bot.send_message(chat_id=update.message.chat_id, text='Bienvenu chez le bot de five!')
    ...
    async def is_admin(self, bot, update):
        """
    ...
    async def add_player(self, update, context):  # pylint:disable=inconsistent-return-statements
        ...
        if user.order_id < 0:
            ...
            await context.bot.send_message(chat_id=update.effective_message.chat_id, text=Msg.name_already_added)
    ...
    async def add_del_forced_player(self, bot, update, args, is_adding):
        """
        ADMIN ONLY: Add forced player
        """
        if not await self.is_admin(bot, update):
            await bot.send_message(chat_id=update.effective_message.chat_id, text=Msg.add_hint)
            return

        cur_chat_id = update.effective_message.chat_id
        for player in ' '.join(args).split(','):
            user, alert_new_user, next_players = self.add_foreign_player(player, is_adding)
    ...
    def add_foreign_player(self, player, is_adding):
        """
        ...
        if is_adding:
            ...
            alert_new_user = len(next_players) > 10
            user.order_id = -1
    ...
    def load_match_info(self):
        ...
        """
        self.match_info = os.path.join(os.path.split(__file__)[0], MATCH_INFO_FILE)
        self.match_info_s3 = os.path.join(self.cube_name, MATCH_INFO_FILE)

        try:
            self.s3_storage.download_file(self.bucket_name, self.match_info_s3, self.match_info)
        except Exception as e:
            logger.error(f'Failed to get the match info file: {str(e)}')
            return
        ...
        if self.match_info and os.path.exists(self.match_info):
            ...
            if content:
                ...
                self.admins_names = [admin.lower() for admin in content.get('admins_names', ['pasha'])[:]]
                logger.info(f'Admins names: {self.admins_names}')

    def load_user_rates(self):
        """
        Load user rates if the file is available
        """
        self.user_rates = os.path.join(os.path.split(__file__)[0], USER_RATES_FILE)
        self.user_rates_s3 = os.path.join(self.cube_name, USER_RATES_FILE)

        try:
            self.s3_storage.download_file(self.bucket_name, self.user_rates_s3, self.user_rates)
        except Exception as e:
            logger.error(f'Failed to get the user rates file: {str(e)}')
            return
    ...
    def save_match_info(self):
        ...
        content['admins_names'] = [admin.lower() for admin in self.admins_names]
        content['cur_players'] = []
    ...
    async def show_add_forbidden_player_keyboard(self, update, context):
        ...
        """
        if await self.is_admin(context.bot, update):
    ...
    async def show_timkeshi_buttons(self, update, context):
        ...
        """
        await self.load_users()
        if update.message.chat.id == MAIN_CHAT_ID:
        ...
        reply_markup = InlineKeyboardMarkup(create_team_color_keyboard())
    ...
    async def on_btn_teamkeshi(self, bot, update):
        ...
        cur_user = self.get_user_from_update(update)
        if query.data in [MotFr.red, MotFr.white]:
            if self.team_keshi.is_finish():
                ...
                if self.team_keshi.is_both_validated():
                    await self.bot.send_message(ADMIN_CHAT_ID, self.team_keshi.print_teams(False, True))
                    ...
                    await self.bot.send_message(ADMIN_CHAT_ID, Msg.validation_finish2.format(captain1, captain2))
                    msg = f'{self.get_next_program()}\n{final_teams}'
                    await self.bot.send_message(ADMIN_CHAT_ID, msg, parse_mode='HTML')
                    self.reset_teams()
                    return
            else:
                self.team_keshi.second_captain_is_red = query.data == MotFr.red
                if cur_user.id == PASHA_USER_ID:
                    cur_user = [user for user in self.all_players if user.id and user.id != PASHA_USER_ID and user.order_id > -1][0]
