...
def create_team_color_keyboard():
    """
    Creates & display RED/WHITE buttons
    """
    keyboard = [[InlineKeyboardButton(MotFr.red, callback_data=MotFr.red),
                 InlineKeyboardButton(MotFr.white, callback_data=MotFr.white)]]
    return keyboard
...
class TeamKeshi():
    ...
    def __init__(self, all_players):
        ...
        self.who_validated = []
        self.second_captain_is_red = False
    ...
    def print_teams(self, sort, show_rates):
        ...
        for captain, players in self.teams.items():
            is_first_capitan = captain == list(self.teams.keys())[0]
            is_white = (is_first_capitan and self.second_captain_is_red) or (not is_first_capitan and not self.second_captain_is_red)
            txt += 3 * '\u26aa' if is_white else 3 * '\U0001f534'
